...
class NextTokenDataset(Dataset):
    def __init__(self, texts, vocab=None, seq_len=8, max_vocab_size=10000):
        ...
        filtered_texts = []
        for text in texts:
            tokens = [t for t in text.split() if t]  # убираем пустые
            if len(tokens) >= 2:
        ...
        for tokens in filtered_texts:
            if len(tokens) <= seq_len:
                # Можно пропустить или использовать — здесь пропускаем для простоты
                continue
